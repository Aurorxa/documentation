...
此测试验证从 DVD.iso 引导时 Anaconda 安装程序是否能正确启动。

## 设置

1. 准备你的系统以引导 DVD.iso 镜像。这可能涉及将镜像写入 USB 密钥或刻录到容量足够的光盘。此外，将 DVD.iso 作为虚拟光盘附加到虚拟机实例，或通过基板管理控制器虚拟介质挂载将 DVD.iso 挂载到服务器应该是可行的，但并非明确要求。
