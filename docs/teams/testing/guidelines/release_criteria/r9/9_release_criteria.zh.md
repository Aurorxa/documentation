...
所有发布阻塞级别安装程序上的 VNC 安装选项必须按预期工作。这意味着启动安装程序或桌面并通过 VNC 连接以完成安装。在企业级 Linux 内核支持的所有系统和硬件类别上，此配置下不得存在阻止连接安装程序的缺陷。

??? tldr "参考资料"
    - 测试用例：
        - [QA:Testcase VNC Graphics Mode](../../../documentation/qa_test_cases/Testcase_VNC_Graphics_Mode.md)
